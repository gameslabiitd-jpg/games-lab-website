...
/**
 * TextFlippingBoard — split-flap mechanical board (à la 21st.dev).
 *
 * Adapted from danielpetho / 21st.dev for the GAMES Lab codebase:
 *   - Import path: `@/lib/utils` → `@/lib/cn` (our project uses cn.ts)
 *   - Otherwise identical: 6×22 grid of split-flap cells, motion v12
 *     spring-eased flips, color-scramble accents during transitions.
 *
 * Accepts either `rows: string[]` (one row per array entry) or
 * `text: string` (auto-wraps + vertically centers). Color codes
 * {R}/{O}/{Y}/{G}/{B}/{V}/{W} render colored panels instead of glyphs.
 */
...
const FlapCell = React.memo(function FlapCell({
  ...
  return (
    <div className="flex aspect-3/5 flex-col overflow-hidden rounded-[2px] border border-neutral-300 md:rounded-[3px] md:border-2 dark:border-black">
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        // Full-bleed by default. No max-w, no rounded corners — meant to
        // be used as a hero / banner that meets the viewport edges.
        // Callers can add rounding + max-w via className when embedding.
        "relative mx-auto w-full bg-neutral-100 p-3 shadow-xl md:p-6 dark:bg-neutral-900 dark:shadow-[0_20px_70px_-15px_rgba(0,0,0,0.6)]",
        className,
      )}
    >
      <div
